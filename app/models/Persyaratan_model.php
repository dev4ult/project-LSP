...
class Persyaratan_model
{
  ...
  {
    $deskripsi = htmlspecialchars($data['persyaratan']);
  ...
  {
    $query = "SELECT id, deskripsi, kategori from list_persyaratan WHERE kategori=:kategori ORDER BY id ASC";
    $this->db->query($query);
  ...
  {
    $deskripsi = htmlspecialchars($data['persyaratan']);
    $query = "UPDATE list_persyaratan SET kategori=:kategori, deskripsi=:deskripsi WHERE id=:id";
    $this->db->query($query);
    $this->db->bind("deskripsi", $deskripsi);
    ...
    try {
      ...
      Flasher::setFlash("Persyaratan tersebut sudah ada di database", 'warning');
    }
  }
