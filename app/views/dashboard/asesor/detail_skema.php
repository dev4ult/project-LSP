      ...
      <?php if (count($data['list-skema-asesi']) > 0) : ?>
        <?php foreach ($data['list-skema-asesi'] as $ds) : ?>
          <tr>
            ...
            <td><?= getAmountDokumenByAsesi($ds['id_asesi']); ?> / <?= getAmountDokumen($ds['id_skema']); ?> Dokumen</td>
            <td><?= $ds['asesmen']; ?></td>
            <td><?= $ds['status_kelulusan']; ?></td>
            <td class="text-center flex items-center justify-around">
              <a href="<?= BASEURL ?>/asesor/penilaian/<?= $ds['id_asesi'] ?>/<?= $ds['id_skema'] ?>" class="btn btn-outline btn-info rounded-md mt-5">Beri Nilai</a>
              <?php if (getStatusAsesmenByIDAsesi($ds['id_skema'], $ds['id_asesi']) == "Sudah") : ?>
                <?php $idAsesi = $ds['id_asesi']; ?>
                <?php $status = $ds['status_kelulusan']; ?>
                <label for="my-modal-<?= $ds['id_asesi'] ?>" class="btn btn-outline btn-success rounded-md mt-5">Info</label>
                ...
                <div class="modal">
                  <div class="modal-box w-[800px] max-w-[800px]">
                    ...
                    <?php if ($status == "Belum Lulus") : ?>
                      <a href="<?= BASEURL ?>/asesor/update_status/<?= $idAsesi ?>/<?= $ds['id_skema'] ?>/1" class="btn btn-success rounded-md mt-5">Lulus</a>
                    <?php else : ?>
                      <a href="<?= BASEURL ?>/asesor/update_status/<?= $idAsesi ?>/<?= $ds['id_skema'] ?>/0" class="btn btn-warning rounded-md mt-5">Tidak Lulus</a>
                    <?php endif; ?>
...
<div class="btn-group my-10">
  <a href="<?= BASEURL ?>/asesor/info/<?= $data['id-skema'] ?>/<?= ($data['page'] == 1) ? 1 : $data['page'] - 1 ?>" class="btn">«</a>
