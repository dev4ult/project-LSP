  ...
  <form action="<?= BASEURL ?>/skema/update/<?= $data['id-skema'] ?>" method="post" class="w-full">
    ...
    <div class="group-ukom-asesi w-full mt-4 flex justify-between items-start">
      <div class="daftar-ukom w-1/2">
        <h1 class="inline-block text-xl font-bold mt-8 mb-5 mr-5 ">Unit Kompetensi Skema</h1><a class="badge badge-info cursor-pointer" href="<?= BASEURL ?>/skema/tambah_kompetensi/<?= $data['id-skema'] ?>">Tambah Kompetensi</a>
              ...
              <?php foreach ($data['list-kompetensi'] as $ds) : ?>
                <tr>
                  ...
                  <td class="text-center flex items-center justify-around">
                    <a href="<?= BASEURL ?>/skema/hapus_kompetensi/<?= $ds['id'] ?>/<?= $data['id-skema'] ?>" class="btn btn-outline btn-warning rounded-md mt-5" onclick="return confirm('Yakin Mau dihapus??')">Hapus</a>
                    <a href="<?= BASEURL ?>/skema/edit_kompetensi/<?= $ds['id'] ?>/<?= $data['id-skema'] ?>" class="btn btn-outline btn-info rounded-md mt-5">Ubah</a>
                  </td>
                </tr>
              <?php endforeach; ?>
            <?php else : ?>
